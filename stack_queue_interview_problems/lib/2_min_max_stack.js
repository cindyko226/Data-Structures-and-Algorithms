...
class MinMaxStack {
    ...
    push(val) {
        ...
        if(!this.minValStack.length || newNode.value <= this.minValStack[this.minValStack.length - 1].value){
            this.minValStack.push(newNode);
        }

        if(!this.maxValStack.length || newNode.value >= this.maxValStack[this.maxValStack.length - 1].value){
            this.maxValStack.push(newNode);
        }
    ...
    pop() {
        ...
        if(this.top.value === this.maxValStack[this.maxValStack.length - 1].value){
            this.maxValStack.pop();
        }
        
        if(this.top.value === this.minValStack[this.minValStack.length - 1].value){
    ...
    min(){
        return this.minValStack[this.minValStack.length - 1] || null;
    }

    max(){
        return this.maxValStack[this.maxValStack.length - 1] || null;
    }
}
